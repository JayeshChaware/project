...
$sql = "SELECT pid, project.eid, pname, duedate, subdate, mark, points, firstName, lastName, base, bonus, total FROM project , erank ,employee, salary  WHERE project.eid = $id AND pid = $pid";
...
if(isset($_POST['update']))
{
 ...
 $result = mysqli_query($conn, "UPDATE erank SET points='$upPoint' WHERE eid=$eid");
 $result = mysqli_query($conn, "UPDATE salary SET bonus='$upBonus' ,total='$upSalar y' WHERE id=$eid");
...
<?php
  $id = (isset($_GET['id']) ? $_GET['id'] : '');
  $pid = (isset($_GET['pid']) ? $_GET['pid'] : '');
  $sql1 = "SELECT pid, project.eid, project.pname, project.duedate, project.subdate, project.mark, erank.points, employee.firstName, employee.lastName, salary.base, salary.bonus, salary.total FROM project , erank ,employee, salary  WHERE project.eid = $id AND project.pid = $pid AND project.eid = erank.eid AND salary.id = project.eid AND employee.id = project.eid AND employee.id = erank.eid";
...
<html>
<head>
  <title>Project Mark | CROWN Corporation</title>
  <!-- Icons font CSS-->
